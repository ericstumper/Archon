...
**Model Validation:**
- Workflows are validated at load time for provider _identity_ only — `provider:` (workflow-level and per-node) must be a registered provider id, otherwise the YAML is rejected with `Unknown provider '<id>'. Registered: ...`.
- Model strings are classified by `resolveModelSpec()` in `packages/workflows/src/model-validation.ts`: tier keywords (`small`/`medium`/`large`) resolve via built-in defaults plus `tiers:` overrides; `@<name>` refs resolve via the merged alias map from config; anything else remains a literal SDK model string and is forwarded verbatim to the resolved SDK.
...
- Gated on `isPerUserProviderKeysEnabled()` (`TOKEN_ENCRYPTION_KEY`); credentials (API keys + subscriptions) injected into runs/chat env at execution time. Subscription tokens refresh-on-read and re-save on rotation. Subscriptions are delivered to native Claude/Codex, Pi, and OMP: native Claude direct chat gets `CLAUDE_CODE_OAUTH_TOKEN`, Codex workflow runs get `CODEX_HOME/auth.json`, Pi workflow runs get per-run `auth.json` via `ARCHON_PI_AUTH_PATH` and env-only direct chat gets `ANTHROPIC_OAUTH_TOKEN` ahead of `ANTHROPIC_API_KEY` (#1984), and OMP's request-env runtime bridge accepts `ANTHROPIC_OAUTH_TOKEN` or `CLAUDE_CODE_OAUTH_TOKEN` before `ANTHROPIC_API_KEY` for anthropic models.
