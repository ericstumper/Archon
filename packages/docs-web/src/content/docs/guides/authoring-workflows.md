...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `provider` | string | inherited | Per-node provider override (any registered provider, e.g. `'claude'`, `'codex'`, `'opencode'`, `'pi'`, `'copilot'`, `'omp'`) |
| `model` | string | inherited | Per-node model override. Tier keywords and `@alias` refs are resolved by Archon; literal model strings are forwarded to the provider. Oh My Pi literals use `<omp-provider-id>/<model-id>` (for example `anthropic/claude-sonnet-4-5` or `openrouter/qwen/qwen3-coder`) |
...
| `allowed_tools` | string[] | — | Whitelist of built-in tools. `[]` = no tools. Supported by Claude, OpenCode, Pi, Copilot, and Oh My Pi (use each provider's tool names; OMP uses `search`, not `grep`) |
| `denied_tools` | string[] | — | Tools to remove. Applied after `allowed_tools`. Supported by Claude, OpenCode, Pi, Copilot, and Oh My Pi |
